...
def simulate(model, model_args = 'lines', rng = None, seed = 42, params=None, dist=None, N=100, transforms = None, subjects = None):
    ...
    Returns:
    class: attributes are .params and .data - .params is an array of transformed parameters used for simulation, .data is a list containing all simulated data.
   ...
   """
    ...
    # return parameters and simulated data
    class output:
        pass
...
def fit(data, nP, fit_func, fit_args, transforms, rng = None, seed = 42, n_jobs = 1, fit = 'EM', EM_iter = 100):
    ...
    Parameters:
    data (list): subjects data in the form of a list of classes. 
    ...
    Returns:
    class: attributes are .m, .s2, .u, .v2 - if fit is 'EM', .m is array of indivudals best fitting parameter estimates, .s2 is individual parameter covariances, .u is group level prior mean estimate, .v2 is group level prior covariance estimate. If fit is 'ML', only .m attribute exists.
   ...
   """
    ...
    # maximum likelihood estimation
        ...
        args = ([getattr(data[x], fit_args[y]) for y in range(len(fit_args))] + [False,m[x,:]] for x in range(N))
        p=Pool(n_jobs)
        ...
        for t in range(EM_iter):
            ...
            # E STEP
            args = ([getattr(data[x], fit_args[y]) for y in range(len(fit_args))] + [False,u,v2,m[x,:],s2[:,x*nP:(x+1)*nP],rng] for x in range(N))
            p=Pool(n_jobs)
            results = list(tqdm.tqdm(p.imap(fit_func,args),desc= 'Participant'))
            p.close()
            p.join()
...
def alpt(data, like_func, fit_args, params, n_jobs = 1):
    ...
    Returns:
    class: attributes are .alpt and .subject_alpts - .alpt is the average likelihood per trial over all subjects, .subject_alpts is a list of average likelihood per trials for each subject.
   """
    ...
    args = ((params[i,:],[getattr(data[i], j) for j in fit_args] + [True]) for i in range(N)) # get required data to pass to function
    p=Pool(n_jobs)
    result = list(tqdm.tqdm(p.starmap(like_func, args),desc = 'Participant')) # calculate average likelihood per trial
    p.close()
    p.join()
    alpts = [result[x][1] for x in range(len(result))] # get list of alpts for each subject
    
    class output:
        pass

    results = output()
    results.alpt = np.mean(alpts) 
    results.subject_alpts = alpts
    return results
...
def iBIC(u, v2, data, transforms, like_func, fit_args, rng= None, seed = 42, Nsamples = 5000, n_jobs = 1):
    ...
    Returns:
    class: attributes are .iBIC, .sublog and .n - .iBIC is the overall integrated Bayesian Information Criterion value, .sublog is a list of subject level fits, .n is the total number of datapoints.
   """
    ...
    for sub in tqdm(range(N), desc = 'Participant'): # iterate over subjects
        ...
        args = ((samples[i,:],[getattr(data[sub], j) for j in fit_args] + [True]) for i in range(Nsamples))
        p=Pool(n_jobs)
